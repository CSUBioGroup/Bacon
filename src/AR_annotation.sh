...
if [ -f "$outDir/${prefix}_result_percent.txt" ]
then
    echo "Remove old file..."
    rm $outDir/${prefix}_result_percent.txt
fi

if [ -f "$outDir/${prefix}_result_AA.txt" ]
then
    echo "Remove old file..."
    rm $outDir/${prefix}_result_AA.txt
fi
...
bedtools intersect -wa -c -a $outDir/${file_name}_loop1.tmp -b $tss > $outDir/${file_name}_loop1_promoter.tmp &&
bedtools intersect -wa -c -a $outDir/${file_name}_loop1.tmp -b $ccres  > $outDir/${file_name}_loop1_enhancer.tmp &&
...
bedtools intersect -wa -c -a $outDir/${file_name}_loop2.tmp -b $tss > $outDir/${file_name}_loop2_promoter.txt &&
bedtools intersect -wa -c -a $outDir/${file_name}_loop2.tmp -b $ccres > $outDir/${file_name}_loop2_enhancer.txt &&
...
echo -e $file_name"\t"$e_e_rate"\t"$e_p_rate"\t"$p_p_rate >> $outDir/${prefix}_result_percent.txt
...
awk '{if($1==$4)print $0}' $file > $outDir/${file_name}_intra.txt
awk '{if($1!=$4)print $0}' $file > $outDir/${file_name}_inter.txt
intra_num=$(< "$outDir/${file_name}_intra.txt" wc -l)
inter_num=$(< "$outDir/${file_name}_inter.txt" wc -l)
...
echo -e $file_name"\t"$intra_num"\t"$inter_num"\t"$active_rate"\t"$inactive_rate"\t"$other_rate >> $outDir/${prefix}_result_AA.txt

echo $file_name": "$all_file_num";"$active_rate"; "$inactive_rate"; "$other_rate

rm $outDir/${file_name}_*
